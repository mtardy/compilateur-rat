...
let string_of_info info =
  ...
  | InfoFun (n,t,tp,bol) -> "Fonction "^n^" : "^(List.fold_right (fun elt tq -> if tq = "" then (string_of_type elt) else (string_of_type elt)^" * "^tq) tp "" )^
                      " -> "^(string_of_type t)^" prototype : "^(string_of_bool bol)
...
let is_prototype i = 
  match !i with
    |InfoFun(_,_,_,b) when b=false -> true
    | _ -> false

let modifier_bol_infoFun i bol = 
  match !i with
    | InfoFun(n,t,lt,_) -> i:= InfoFun(n,t,lt,bol)
    | _ -> failwith "Appel modifier_bol_infoFun pas sur un infoFun"
