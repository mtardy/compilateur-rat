    ...
    begin
      ...
      match chercherGlobalement tds nom with
        ...
        | Some infoFun -> 
            ...
            AppelFonction(infoFun, lexprTds)
    end
...
let rec newArg tds (typ,nom) = 
  match chercherLocalement tds nom in
    | None -> let info = InfoVar (nom,Undefined, 0, "") in 
              ajouter tds nom info;
              (typ,info)
    | Some _ -> raise (IdentifiantNonDeclare nom)
...
let analyse_tds_fonction maintds (AstSyntax.Fonction(t,n,lp,li,e))  =
  let tdsArgs = creerTDSFille tds in
  let infoListArgs = List.map (fun x -> newArg tds x) lp in
  (*TO DO: Tout sauf les variables *)
  Fonction (typ,info_ast,(typ,info_ast)list,instruction,expression)  
